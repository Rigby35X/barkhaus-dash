...
export function InlineEditor() {
      ...
      if (t === 'studio:save') {
        ;(async () => {
          try {
            ...
            if (res.ok) dispatch({ type: 'SAVE_CHANGES' })
          } catch {}
        })()
      }
      if (t === 'studio:togglePreview') {
        dispatch({ type: 'TOGGLE_PREVIEW' })
      }
      if (t === 'studio:publish') {
        ;(async () => {
          try {
            const res = await fetch(`/api/orgs/${orgId}/publish`, withXanoAuth({
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ pageSlug: state.pageSlug }),
            }))
            if (!res.ok) {
  ...
  }, [state.isEditing, state.pageSlug, state.elements, state.userId, orgId])
  ...
  const handleSave = async () => {
    try {
      const res = await fetch(
        `/api/orgs/${orgId}/pages/${state.pageSlug}/elements`,
        withXanoAuth({
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ elements: state.elements, userId: state.userId }),
